...
try {
    ...
    if ($valid) {
        ...
        if (isset($_FILES['screenshot']) && is_uploaded_file($_FILES['screenshot']['tmp_name'])) {
            ...
            $sql = mysqli_query($conn, "UPDATE orders SET status = '$status', date = '$currentDateTime', payment_term='$paymentTerm', payment_mode='$paymentMode', receipt = '$receipt', reference_number='$referenceInput', screenshot='$img', date='$currentDateTime' WHERE tran_id = '$tran_id' ");
        }else {
            $sql = mysqli_query($conn, "UPDATE orders SET status = '$status', date = '$currentDateTime', payment_term='$paymentTerm', payment_mode='$paymentMode', receipt = '$receipt', reference_number='$referenceInput', date='$currentDateTime' WHERE tran_id = '$tran_id' ");
        }
        ...
        // $sql223 = mysqli_query($conn, "INSERT INTO notifications(messageTo, message, date, transaction, status) VALUES('staff','$messageStaff','$date','$tran','$status')");
